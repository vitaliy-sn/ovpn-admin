...
func (openVPNPKI *OpenVPNPKI) initPKI() (err error) {
	if res, _ := openVPNPKI.secretCheckExists(secretPKI); res {
		cert, err := openVPNPKI.secretGetClientCert(secretPKI)
		if err != nil {
			return err
		}
...
func (openVPNPKI *OpenVPNPKI) indexTxtUpdate() (err error) {
	...
	data := map[string][]byte{"index.txt": []byte(indexTxt)}

	if res, _ := openVPNPKI.secretCheckExists(secretIndexTxt); !res {
		err = openVPNPKI.secretCreate(metav1.ObjectMeta{Name: secretIndexTxt}, data)
	} else {
		err = openVPNPKI.secretUpdate(metav1.ObjectMeta{Name: secretIndexTxt}, data)
	}
